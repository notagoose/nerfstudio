...
class FullDataManagerConfig(DataManagerConfig):
    ...
    _target: Type = field(default_factory=lambda: FullDataManager)
    """Target class to instantiate."""
    dataparser: AnnotatedDataParserUnion = BlenderDataParserConfig()
    """Specifies the dataparser used to unpack the data."""
    train_num_rays_per_batch: int = -1
...
class FullDataManager(DataManager, Generic[TDataset]):
    ...
    config: FullDataManagerConfig
    train_dataset: TDataset
    eval_dataset: TDataset
    train_dataparser_outputs: DataparserOutputs
    train_pixel_sampler: Optional[PixelSampler] = FullPixelSampler
    eval_pixel_sampler: Optional[PixelSampler] = FullPixelSampler
    ...
    def _get_pixel_sampler(self, dataset: TDataset, *args: Any, **kwargs: Any) -> PixelSampler:
        ...
        cameras = self.train_dataparser_outputs.cameras
        image_height = cameras[0].height.item() if self.config.image_height[0] == -1 else self.config.image_height[0]
        image_width = cameras[0].width.item() if self.config.image_width[0] == -1 else self.config.image_width[0]
        print("HOW MANY CAMERAS", len(self.cameras))
        return FullPixelSampler(*args, **kwargs, image_height=image_height, image_width=image_width)
